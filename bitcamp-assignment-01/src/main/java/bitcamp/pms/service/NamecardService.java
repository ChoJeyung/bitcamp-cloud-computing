...
import bitcamp.pms.dao.NamecardDao;
import bitcamp.pms.domain.Namecard;

@Service
public class NamecardService {
    
    @Autowired NamecardDao namecardDao;

    public List<Namecard> list(int page, int size) {
        // DB에서 가져올 데이터의 페이지 정보
        HashMap<String,Object> params = new HashMap<>();
        params.put("startIndex", (page - 1) * size);
        params.put("pageSize", size);
        
        return namecardDao.selectList(params);
    }

    public Namecard get(String name) {
        return namecardDao.selectOne(name);
    }
    ...
    public int update(Namecard namecard) {
        int count = namecardDao.update(namecard);
        return count;
    }

    public int delete(String name) {
        return namecardDao.delete(name);
    }

    public void add(Namecard namecard) {
    	namecardDao.insert(namecard);
    }
    
    public int getTotalPage(int size) {
        int count = namecardDao.countAll();
